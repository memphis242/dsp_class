...
void TimerInt_Init(uint32_t ldval){	
	...
	PIT->CHANNEL[0].LDVAL	=	PIT_LDVAL_TSV(ldval)	;		//Set Load value to XXXX clock cycles = YYYYY ms. --> For 10ksamples/s --> 100us per sample, with 60MHz bus clock, we need 6000 - 1 load value
